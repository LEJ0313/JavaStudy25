...
public class ExSwitchExam2 {

	public static void main(String[] args) {
		//향상된 switch문은 같은 조건을 동시에 처리하기 위한 기법
		//case뒤에 여러 값이 존재할 수 있다.
		...
		System.out.println(Monster + "는" + kind + "그룹에 추가된 동물입니다.");
	}//main 메서드 종료

	//추가 메서드 활용 (동작부분을 따로만듬 -> 관심사의 분리(oop핵심))
	static String WhoIsIt(String Monster) {
		//매개값으로 main()넘어온 문자열을 처리
		String kind = "미지의 생물"; //출력용 변수 선언 및 초기값 입력
	switch(Monster) {
		case "호랑이", "사자", "강아지", "고양이", "고래" -> kind = "포유류" ;
		//향상된 switch문에서는 break를 사용할 수 없다.
		case "독수리", "참새", "비둘기", "까마귀", "닭" -> kind = "조류" ;
