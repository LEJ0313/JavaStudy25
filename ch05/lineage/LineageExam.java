...
public class LineageExam {
	...
	public static Account[] accouts = new Account[10];//로그인용 배열
	public static Account loginAccount ;//로그인 성공시 객체(세선역할)
	...
	static {
		...
		accouts[0] = account; //배열 0번에 계정객체를 연결
	}//db대신 초기값 지정(생성자 대신)
	...
	public static void main(String[] args) {
		...
		boolean run = true;
		while(run) {
			System.out.println("1. 로그인 | 2. 캐릭터선택 | 3. 게임 실행 | 4. 종료");
			System.out.print(">>>");
			...
			switch(select){
				case 1 :
					...
					loginAccount = LoginService.menu(scanner, accouts, loginAccount);
					break ;
